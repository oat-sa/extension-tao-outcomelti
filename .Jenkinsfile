    ...
    stages {
        stage('Resolve TAO dependencies') {
            environment {
                ...
                REPO_NAME='oat-sa/extension-tao-outcomelti'
            }
            steps {
                sh(
        ...
        stage('Tests') {
            parallel {
                stage('Backend Tests') {
                    when {
                        expression {
                            fileExists('build/taoLtiBasicOutcome/test/unit')
                        }
                    }
                    agent {
                        docker {
                    ...
                    steps {
                        dir('build'){
                            sh(
                                ...
                                script: './vendor/bin/phpunit taoLtiBasicOutcome/test/unit'
                            )
                        }
                    }
                }
                stage('Frontend Tests') {
                    when {
                        expression {
                            fileExists('build/taoLtiBasicOutcome/views/build/grunt/test.js')
                        }
                    }
                    agent {
                        docker {
                    ...
                    steps {
                        dir('build/tao/views/build') {
                            ...
                            sh (
                                ...
                                script: 'npx grunt connect:test taoltibasicoutcometest'
                            )
                        }
                    }
                }
